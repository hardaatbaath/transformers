...
class BilingualDataset(Dataset):
    ...
    def __getitem__(self, idx: Any) -> Any:
        ...
        dec_num_padding_tokens = self.seq_len - len(dec_input_tokens) - 1 # We will add only [SOS]
        ...
        # Add only [SOS] token to the decoder input text
        ...
        # Add only [EOS] token to the lable text (what we expect from the decoder)
        label = torch.cat(
            [
                torch.tensor(dec_input_tokens, dtype = torch.int64),
                self.eos_token,
